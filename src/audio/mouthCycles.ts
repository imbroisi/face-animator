...
export type MouthOptions = {
  triggerOpening: number; triggerClosing: number; fadeout: number; linearFadeout: boolean;
  openedTime: number; fadein: number; linearFadein: boolean; closedTime: number;
}
export type MouthTimeline = {
  starts: number[]; closingStarts: number[]; faceIndices: number[]; duration: number; options: MouthOptions;
}
...
export function buildMouthCycles(
  ...
  const options = { triggerOpening: TRIGGER_OPENING, triggerClosing: TRIGGER_CLOSING, fadeout: FADEOUT, linearFadeout: LINEAR_FADEOUT,
    openedTime: OPENED_TIME, fadein: FADEIN, linearFadein: LINEAR_FADEIN,
    closedTime: CLOSED_TIME, ...overrides }
  ...
  for (const value of [options.triggerOpening, options.triggerClosing]) {
    if (!Number.isFinite(value) || value < 0 || value > 100) throw new Error('Os triggers devem estar entre 0 e 100.')
  }
  const durations = [options.fadeout, options.openedTime, options.fadein, options.closedTime]
  if (durations.some(value => !Number.isFinite(value) || value < 0)) throw new Error('Os tempos devem ser não negativos.')
  ...
  if (peak > 0 && faceIndices.length > 1) {
    for (let sample = 0; sample < mono.length;) {
      ...
      if (amplitude > 0 && amplitude >= openingThreshold) {
        ...
        const start = sample
        let closingSample = sample + Math.ceil((options.fadeout + options.openedTime) / 1000 * sampleRate)
        ...
        sample = Math.max(start + 1, closingSample + Math.ceil((options.fadein + options.closedTime) / 1000 * sampleRate))
      } else sample++
    }
  }
...
export function cycleFaceAt(timeline: MouthTimeline | null, time: number) {
  ...
  if (elapsed < options.fadeout) {
    return faceIndices[options.linearFadeout ? Math.floor(topMouth * elapsed / options.fadeout) : topMouth]
  }
  const closing = (time - closingStarts[high]) * 1000 + 1e-7
  if (closing < 0) return faceIndices[topMouth]
  if (closing < options.fadein) {
    return faceIndices[options.linearFadein ? Math.ceil(topMouth * (1 - closing / options.fadein)) : 0]
  }
  return 0
}
